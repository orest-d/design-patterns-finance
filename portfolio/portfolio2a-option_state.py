...
class VanillaOption(Asset):
    def __init__(self, strike, maturity, asset, call_option=True):
        self.strike = strike
        self.asset = asset
    ...
    def price(self, scenario):
        ...
        if self.call_option:
            ...
            return K * np.exp(-r * T) * si.norm.cdf(-d2, 0.0, 1.0) - S * si.norm.cdf(
                -d1, 0.0, 1.0
            )
...
class VanillaOption(Asset):                                              # State pattern - like proxy depending on a state
    def __init__(self, strike, maturity, asset, call_option=True):
        self._init_option(call_option)

    def _init_option(self, call_option):
        if call_option:
            self.option = VanillaCallOption(strike, maturity, asset)
        else:
            self.option = VanillaPutOption(strike, maturity, asset)

    def price(self, scenario):
        return self.option.price(scenario)

    @property
    def call_option(self):
        return isinstance(self.option, VanillaCallOption)

    @call_option.setter
    def call_option(self, call_option):
        self._init_option(call_option)
